class RollingHash:
    ...
    def slide_window(self):
        ...
        if self.window_end <= len(self.text) - 1:
            ...
            # add the rolling hash. for e.g. H(bcd) = H(abc-a) * 26 + H(d)
            self.hash *= 26
...
def rabin_karp(word_to_find, text_to_search):
    ...
    rolling_hash = RollingHash(text_to_search, len(word_to_find))
    # computes the word hash of the sliding window
    word_hash = RollingHash(word_to_find, len(word_to_find))
    # word_hash.move_window()

    for i in range(len(text_to_search) - len(word_to_find) + 1):
        if rolling_hash.hash == word_hash.hash:
            if rolling_hash.window_text() == word_to_find:
