# -*- coding: utf-8 -*-
"""
Created on Sat Feb  9 12:28:25 2019
@author: sagar

input_sequence of size n, there are 3 types of queries on this array.
query : to print the minimum in the sub-array
update : to update the value at the specified index
tree : prints the segment tree in array form
"""
...
if __name__ == '__main__':
    ...
    for i in range(q):
        query_type, left, right = input().strip().split()
        if query_type == 'min':
            print(seg_tree.minimum(int(left) - 1, int(right) - 1))
        elif query_type == 'update':
            seg_tree.update(int(left) - 1, int(right), 0, n - 1, 0)
